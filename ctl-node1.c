...
void on_status_clicked(GtkWidget *widget, gpointer label) {
    system("sudo virsh list --all | grep node1 | awk '{print $3}' > /tmp/.vm1_status.txt");
    FILE *fp = fopen("/tmp/.vm1_status.txt", "r");
    if (fp) {
        char status[100];
...
int main(int argc, char *argv[]) {
    ...
    status_button = gtk_button_new_with_label("Status node1 VM");
    ...
    shutdown_button = gtk_button_new_with_label("Shutdown node1 VM");
    ...
    hard_reboot_button = gtk_button_new_with_label("Hard Reboot node1 VM");
    ...
    console_button = gtk_button_new_with_label("Console node1 VM");
    ...
    rebuild_button = gtk_button_new_with_label("Rebuild node1 VM");
